...
- 梳理完整单位链：SP3(km/μs) ↔ 内部(mm/s) ↔ 误差(m)

**Bug 修复（3 个，不影响数据复刻）**
- `readsp3.m` 第 7 行：`any(errmsg)` → `fid == -1`（错误检查逻辑修正）
- `writesp3.m` 第 8 行：`any(errmsgin) || any(errmsgout)` → `fin == -1 || fout == -1`
...
## 目录结构与文件说明
    ...
    │
    ├── readsp3.m               # SP3 文件读取函数：解析头部信息、epoch 数据、卫星位置/钟差
    ├── writesp3.m              # SP3 文件写入函数：将含误差的数据按 SP3-c 格式输出
    ├── sp3p2sp3v.m             # SP3 位置到速度：9 阶 Lagrange 插值 + 地球自转改正求速度
...
### 总体流程
    ...
    输入 SP3 (whu22924.sp3 / GLwhu22924.sp3)
            |
            v
        readsp3.m  -->  sp3p 结构体（位置 + 钟差，ECEF，毫米/微秒）
            |
            v
...
- 位置单位：SP3 中为 km，读取后转为毫米（x1000）
- 钟差单位：SP3 中为微秒，读取后转为秒（x10^-6）
- 输出结构体 sp3p：recef[epoch, 4, sat]（x,y,z,clk，单位 mm/s）
...
- z 分量不受地球自转影响，直接插值
- 输出 sp3v.vecef[epoch, 3, sat]，单位 mm/s
...
| 输入文件 | 输出文件 | 版本 | 说明 |
|---------|---------|------|------|
| whu22924.sp3 | Cwhu22924.sp3 | V1 | 所有卫星添加误差 |
| whu22925.sp3 | Cwhu22925.sp3 | V1 | DOY 349 |
| whu22926.sp3 | Cwhu22926.sp3 | V1 | DOY 350 |
| GLwhu22924.sp3 | CGLwhu22924.sp3 | V2 | 仅 LEO 添加误差 |
| whu22924.sp3 | Swhu22924.sp3 | - | 疑为备份/中间文件 |
| whu22925.sp3 | Swhu22925.sp3 | - | DOY 349 |
