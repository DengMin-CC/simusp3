...
**新增**
- `交付文档.md`：三层结构交付文档，面向新接手师弟
  - 第一部分"怎么跑"：一句话定位 + 一条命令 + 输出解读 + 手动操作流程
  - 第二部分"跑出什么"：文件规格、单位链、验证方法、性能参考
  - === 分割线：日常使用者到此可停 ===
  - 第三部分"工作原理"：算法流程、误差模型、V1/V2 差异、坐标转换、随机数设计
  - 第四部分"配置与维护"：6 条硬约束、完整错误排查、文件全景、调用链、踩坑备忘录
  - 完整保留 v1 的信息量，按"怎么跑→跑出什么→原理→细节"四段组织
...
## 目录结构与文件说明
    ...
    │
    ├── 交付文档.md             # [交付] 交付文档，新接手人入口（怎么跑→规格→原理→维护）
    │
